...
const Groups = (props: GroupsProps) => {
  ...
  const fetchGroupList = async () => {
    ...
    try {
      res = await fetch(
        `/dashboard/api/groupchat?id=${props.currentUser.user_id}`,
        {
          method: "GET",
        }
      );
  ...
  return (
    <section className="flex flex-row size-full ">
      ...
      <section className="size-full bg-dark-4 flex justify-center"><p className="text-[20px] text-gray-2 self-center">No group chat selected</p></section>
    </section>
  );
};
